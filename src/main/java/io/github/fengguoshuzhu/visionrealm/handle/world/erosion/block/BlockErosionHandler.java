package io.github.fengguoshuzhu.visionrealm.handle.world.erosion.block;
...
public class BlockErosionHandler {
    ...
    public static boolean tryErosion(Block block, Level level, BlockPos pos, ErosionType type) {
        return process(block, level, pos, type, source -> {
            if (source instanceof CanBeErosion) {
                return (CanBeErosion<Block, ?, ?>) source;
            } else {
                return null;
            }
        });
    ...
    public static boolean tryErosion(BlockErosionKey<?, ?> key, Level level, BlockPos pos, ErosionType type) {
