package io.github.fengguoshuzhu.visionrealm.api.event.data.erosion.block;
...
public class BlockErosionLoaderRegisterEvent extends Event {
     ...
     * @param codec The codec used to decode JSON files into {@link BlockErosionKey} instances
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    public void registryLoader(String path, Codec codec) {
        try {
            this.pendingLoaders.add(new PendingLoader(path, (Codec<BlockErosionKey<?, ?>>) codec));
        } catch (Exception e) {
     ...
     * @param codec The codec for decoding {@link BlockErosionKey} instances
     */
    public record PendingLoader(String path, Codec<BlockErosionKey<?, ?>> codec) {
    }
}
