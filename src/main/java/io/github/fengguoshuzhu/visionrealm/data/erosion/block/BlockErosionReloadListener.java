package io.github.fengguoshuzhu.visionrealm.data.erosion.block;
...
public class BlockErosionReloadListener extends SimplePreparableReloadListener<Collection<Map<Block, BlockErosionKey<Object, Object>>>> {
    ...
    @NotNull
    @Override
    protected Collection<Map<Block, BlockErosionKey<Object, Object>>> prepare(@NotNull ResourceManager resourceManager, @NotNull ProfilerFiller profiler) {
        BlockErosionLoaderRegisterEvent event = new BlockErosionLoaderRegisterEvent();
        NeoForge.EVENT_BUS.post(event);

        List<Map<Block, BlockErosionKey<Object, Object>>> result = new ArrayList<>();
    ...
    @Override
    protected void apply(@NotNull Collection<Map<Block, BlockErosionKey<Object, Object>>> map, @NotNull ResourceManager resourceManager, @NotNull ProfilerFiller profiler) {
        this.manager.loadFromMap(map);
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    protected <T, R> Codec<BlockErosionKey<T, R>> conversionCodec(Codec<BlockErosionKey<?, ?>> codec) {
        // 转换为具体类型的 Codec（虽然是 unchecked，但安全）
        // Convert to concrete type Codec (unchecked but safe)
        return (Codec<BlockErosionKey<T, R>>) (Codec) codec;
    }

    protected <T, R> Map<Block, BlockErosionKey<T, R>> loadErosionKeys(@NotNull ResourceManager resourceManager, @NotNull String path, @NotNull Codec<BlockErosionKey<T, R>> codec) {
        Map<Block, Collection<BlockErosionKey<T, R>>> map = new HashMap<>();
        resourceManager.listResources(path, fileName -> fileName.getPath().endsWith(".json"))
                .forEach((fileId, resource) -> {
                    try (var reader = resource.openAsReader()) {
                        BlockErosionKey<T, R> erosionKey = codec.decode(JsonOps.INSTANCE, JsonParser.parseReader(reader))
        ...
        Map<Block, BlockErosionKey<T, R>> newMap = new HashMap<>();
    ...
    protected <T, R> BlockErosionKey<T, R> merger(Collection<BlockErosionKey<T, R>> keys) {
        BlockErosionKey<T, R> presentKey = null;
        for (BlockErosionKey<T, R> key : keys) {
            if (presentKey == null) {
                presentKey = key;
            } else {
