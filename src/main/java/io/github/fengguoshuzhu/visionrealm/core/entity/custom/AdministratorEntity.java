...
import io.github.fengguoshuzhu.visionrealm.core.entity.ai.goal.entity.administrator.InterferenceGoal;
import io.github.fengguoshuzhu.visionrealm.core.entity.ai.goal.entity.administrator.ObservationGoal;
...
public class AdministratorEntity extends PathfinderMob implements EntityParticleProvider, LivingEntityExtensions, SecondaryTarget {
    ...
    @Nullable
    @Override
    public ParticleConfig getHurtParticles(DamageSource source, float amount) {
        return ParticleConfig.create(new BlockParticleOption(ParticleTypes.BLOCK, ModBlocks.CustomBlocks.ERROR_BLOCK.get().defaultBlockState()),
                5, 0, 0.65, 1.0, 0.65, 0, 1.5, 0);
    }

    @Nullable
    @Override
    public ParticleConfig getTickParticles() {
        return ParticleConfig.create(ModParticleTypes.ERROR_PARTICLE_1.get(), this.getBehaviorMode().getParticleCount(),
