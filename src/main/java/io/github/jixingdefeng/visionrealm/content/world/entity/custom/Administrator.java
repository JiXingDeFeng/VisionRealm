...
import io.github.jixingdefeng.visionrealm.content.world.entity.ai.goal.entity.administrator.InterferenceGoal;
import io.github.jixingdefeng.visionrealm.content.world.entity.ai.goal.entity.administrator.ObservationGoal;
...
public class Administrator extends PathfinderMob implements EntityParticleProvider, SecondaryTarget {
    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(VisionRealm.MOD_ID, "textures/entity/administrator/administrator.png");
    public static final String[] NAMES = {
            "Administrator",
            "UnknownUser316"
    };
    protected static final EntityDataAccessor<Integer> MODE = SynchedEntityData.defineId(Administrator.class, EntityDataSerializers.INT);
    protected static final EntityDataAccessor<Integer> TELEPORT_TARGET_PHASE =  SynchedEntityData.defineId(Administrator.class, EntityDataSerializers.INT);
    ...
    public Administrator(EntityType<? extends Administrator> type, Level level) {
        super(type, level);
        this.isReally = false;
    ...
    public static void initEntity(@NotNull Administrator entity, Player player, @NotNull BehaviorMode behaviorMode) {
    ...
    public SingletonParticleConfig getHurtParticles(DamageSource source, float amount) {
        return SingletonParticleConfig.create(new BlockParticleOption(ParticleTypes.BLOCK, ModBlocks.ERROR_BLOCK.get().defaultBlockState()),
    ...
    @Override
    public int getTickParticleInterval() {
    ...
    public boolean immunityTheDamage(DamageSource source) {
        if (Config.ADMINISTRATOR_VULNERABLE.get()) {
            return false;
        } else {
            if (source.is(DamageTypes.GENERIC_KILL)) {
                return false;
            } else if (this.canBeAttacked()) {
                boolean notImmune = source.is(DamageTypes.GENERIC)
    ...
    public void setTeleportTargetStage(int AnimationPhase) {
        this.getEntityData().set(Administrator.TELEPORT_TARGET_PHASE, AnimationPhase);
    }

    public void setBehaviorMode(BehaviorMode behaviorMode) {
        this.resetState(this.getBehaviorMode());
        this.getEntityData().set(Administrator.MODE, behaviorMode.getId());
    ...
    public int getTeleportTargetStage() {
        return this.getEntityData().get(Administrator.TELEPORT_TARGET_PHASE);
    }
