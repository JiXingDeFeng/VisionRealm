package io.github.jixingdefeng.visionrealm.common.erosion.manager.infection.block;
 ...
 *   <li>Lazy initialization with async datapack loading support</li>
 * </ul>
 *
 * @author JiXingDeFeng
 ...
 * @since 0.0.1-dev
 */
public class BlockErosionEntryManager {
    public static final String PATH = "erosion/block_entry";
    ...
    private static BlockErosionEntryManager INSTANCE;
    private final Object2ObjectOpenHashMap<Block, ArrayWeightRandomList<BlockErosionEntry<?, ?>>> EROSION_ENTRY = new Object2ObjectOpenHashMap<>();
     ...
     */
    public static Optional<BlockErosionEntryManager> getInstance() {
    ...
    /**
     * Initializes the manager with erosion key data from data pack loading.
     * <p>
     * This method processes the loaded erosion key data directly and updates the internal cache.
     *
     * @param collection A collection of maps containing block-to-erosion-key mappings
     */
    public void start(Collection<Map<Block, BaseBlockErosionEntry<Object, Object>>> collection) {
        if (this.EROSION_ENTRY.isEmpty()) {
            this.addEntry(collection);
            this.open();
        } else {
     ...
     * @return {@code true} if at least one erosion key exists (either from data pack or hardcoded)
     */
    public boolean containsKey(Block source) {
        return this.EROSION_ENTRY.containsKey(source) || source instanceof CanBeErosionBlock;
    }

    /**
     * Retrieves a random erosion key that is applicable at the specified position.
     * <p>
     ...
     * @return An applicable erosion key, or {@code null} if none available
     */
    @Nullable
    ...
    /**
     * Retrieves a random erosion key from the data pack for the specified source block.
     * <p>
    ...
    public CanBeErosionBlock<?, ?> get(Block source, boolean useHardcoded) {
        ArrayWeightRandomList<BlockErosionEntry<?, ?>> randomList = this.EROSION_ENTRY.get(source);
    ...
    /**
     * Processes and stores the loaded erosion key data.
     * <p>
    ...
    protected void addEntry(Collection<Map<Block, BaseBlockErosionEntry<Object, Object>>> collection) {
        Runnable task = () -> {
            ...
            this.EROSION_ENTRY.ensureCapacity(entryMap.size());
            entryMap.forEach((block, list) ->
                    this.EROSION_ENTRY.put(block, ArrayWeightRandomList.create(list))
            );
        };
